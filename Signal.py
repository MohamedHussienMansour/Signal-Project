...
def apply_filter(signal, filter_type='lowpass', cutoff_frequency=500, sample_rate=44100):
    # Apply a filter (lowpass or highpass)
    if filter_type == 'lowpass':
        filter_mask = np.zeros_like(signal)
        filter_mask= 1/np.sqrt(1+(np.fft.fftfreq(len(signal), d=1/sample_rate)/cutoff_frequency)**2)
    elif filter_type == 'highpass':
        filter_mask = np.zeros_like(signal)
        filter_mask= 1/np.sqrt(1+(cutoff_frequency/np.fft.fftfreq(len(signal), d=1/sample_rate))**2)
    else:
        raise ValueError("Invalid filter type. Use 'lowpass' or 'highpass'.")

    # Apply the filter in the frequency domain
    filtered_fft = np.fft.fftshift(np.fft.fft(signal)) * filter_mask
...
Names_of_team_members=["Mohamed Hussien Mansour Sayed","Mohamed Sayed Ahmed Sayed","Yousef Ahmed Mohamed","Mahmoud Hazem","Yousef Mahmoud Ahmed Amin"]

for i in range(5):

    # Specify the path to your audio file
    audio_file_path = f"Downloads/Voice_{Names_of_team_members[i]}.ogg"

    # Read the original audio file
    original_signal, sample_rate = read_audio_file(audio_file_path)

    # Task 1: Plot the original audio signal in the time domain
    plot_time_domain(original_signal, sample_rate, title=f"Original Time Domain Signal of {Names_of_team_members[i]}")

    # Task 2: Plot the frequency domain representation of the original signal
    plot_frequency_domain(original_signal, sample_rate, title=f"Original Frequency Domain Signal of {Names_of_team_members[i]}")
    ...
    plot_frequency_domain(filtered_signal, sample_rate, title=f"Filtered Frequency Domain Signal of {Names_of_team_members[i]}")

    # Task 4: Find the corresponding signal in time domain for the filtered signal and plot it
    plot_time_domain(filtered_signal, sample_rate, title=f"Filtered Time Domain Signal {Names_of_team_members[i]}")

    # Task 5: Save the filtered signal in time domain as an audio file
    filtered_audio_file_path = f"Downloads/Voice_{Names_of_team_members[i]}_filtered.ogg"
